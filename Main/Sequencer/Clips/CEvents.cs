...
namespace AnimFlex.Sequencer.Clips
{
    ...
    public abstract class CEvent<T1, T2> : Clip
    {
        ...
        public VariableFetch<T1> Value1;
        public VariableFetch<T2> Value2;

        protected override void OnStart()
        {
            InjectVariable(ref Value1);
            InjectVariable(ref Value2);
            Event.Invoke(Value1.value, Value2.value);
            PlayNext();
        }
        public override void OnEnd() { }
    }
    ...
    public abstract class CEvent<T1, T2, T3> : Clip
    {
        ...
        public VariableFetch<T1> Value1;
        public VariableFetch<T2> Value2;
        public VariableFetch<T3> Value3;

        protected override void OnStart()
        {
            InjectVariable(ref Value1);
            InjectVariable(ref Value2);
            InjectVariable(ref Value3);
            Event.Invoke(Value1.value, Value2.value, Value3.value);
            PlayNext();
        }
        public override void OnEnd() { }
    }
