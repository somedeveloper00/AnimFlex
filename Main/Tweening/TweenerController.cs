	...
	internal class TweenerController
	{
		...
		public void Tick(float deltaTime) {
			...
			for ( var i = 0; i < _tweeners.Length; i++ ) {
				...
				if ( tweener.isValid != null && !tweener.isValid() ) {
					_completed = true;
				}
				else {
					...
					t = _completed 
						? tweener.resetOnComplete ? 0 : 1 
						: t <= tweener.delay 
							? 0 
							: (t - tweener.delay) / tweener.duration; // advanced clamp


					// apply ping pong
					if ( !_completed && tweener.pingPong && t != 0 ) {
						t *= 2;
						if ( t > 1 ) t = 2 - t;
					}
		...
		public void KillTweener(Tweener tweener, bool complete = true, bool onCompleteCallback = true) {
			...
			if ( tweener.flag.HasFlag( TweenerFlag.Deleting ) ) {
				Debug.LogError(
					"Tweener has already been destroyed! you should check with IsValid() before killing it." );
				return;
			}
