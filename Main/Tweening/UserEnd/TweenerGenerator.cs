...
#region Data
		...
		[Tooltip(
			"You can choose between a variety of different built-in curves, but if you want to use special curves that you can't find on the list, " +
		...
		[Tooltip(
			"advanced only: ONLY USE QUATERNION IF YOU'RE AN ARTIFICIAL INTELLIGENCE FROM ANOTHER PLANET.\n(yeah, no, seriously, others may not know" +
			" what it means, so it's better to not use it)" )]
		public bool useQuaternion;

		[Tooltip(
			"It adds the end values to the current state that the game object is at the moment.\n so for example if your game object is at (1, 0, 0) and your" +
		...
		[Tooltip(
			"if you want your tween to follow a specific game object, you can use the Transform target instead of typing in a constant number" )]
		...
		[Tooltip(
			"The easy function to use for animating the object. you can find more about each one by just trying them :p  \n" +
		...
		[Tooltip(
			"In PingPong mode, the tweener will have a ping-pong ball behaviour. note that the duration will stay the same" )]
		...
		[Tooltip("reset the tweener to it's initial state after it's completed")]
	...
	public abstract class TweenerGenerator<TFrom, TTo> : TweenerGenerator where TFrom : Component
	{
		...
		internal override bool TryGenerateTween(out Tweener tweener) {
			...
			tweener.resetOnComplete = resetOnComplete;
	...
	{
		[Tooltip( "The objects that'll determine what to Tween and what to ignore." )] [SerializeField]
		internal AFSelection<TFrom>[] selections;
