	...
	public class TweenerGeneratorEditor : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			using (new AFStyles.GuiColor( Color.white )) {
				...
				if ( !AFPreviewUtils.isActive && !Application.isPlaying ) {
					...
					DrawToggles( position );

					position.y += DrawToggles_Height() + AFStyles.VerticalSpace;
		...
		private float DrawToggles_Height() => AFStyles.Height;

		private void DrawToggles(Rect position) {
			var resetOnComplete = property.FindPropertyRelative( nameof(TweenerGenerator.resetOnComplete) );
			float h = position.height;
			position.height = AFStyles.Height;
			position.x += 2;
			position.width -= 2;
			EditorGUI.DrawRect( position, AFStyles.BoxColorDarker );
			position.height = h;
			using (new AFStyles.EditorLabelWidth( 120 ))
				EditorGUI.PropertyField( position, resetOnComplete );
			position.x -= 2;
		}
		...
		protected virtual void DrawPlayback(Rect position) {
			...
			if ( AFPreviewUtils.isActive ) {
				using (new AFStyles.GuiForceActive())
					if ( GUI.Button( pos, "Stop", AFStyles.Button ) )
						AFPreviewUtils.StopPreviewMode();
			}
			else {
				using (new EditorGUI.DisabledScope( Application.isPlaying ))
					if ( GUI.Button( pos, "Play Tweener", AFStyles.Button ) )
		...
		protected virtual void DrawUnityEvents(Rect position) {
			...
			switch (selectedEvent) {
				case 0:
					EditorGUI.PropertyField( pos, onStart, GUIContent.none );
					break;
				case 1:
					EditorGUI.PropertyField( pos, onUpdate, GUIContent.none );
					break;
				case 2:
					EditorGUI.PropertyField( pos, onComplete, GUIContent.none );
					break;
				case 3:
					EditorGUI.PropertyField( pos, onKill, GUIContent.none );
					break;
			}
		}
		...
		protected virtual void DrawLoop(Rect position) {
			...
			using (new EditorGUI.DisabledGroupScope( loopsProp.intValue == 0 )) {
				pos.x += pos.width;
				using (new AFStyles.EditorLabelWidth( 80 ))
					EditorGUI.PropertyField( pos, loopDelayProp,
						new GUIContent( "Loop-Delay", loopDelayProp.tooltip ) );
			}
		...
		protected virtual void DrawValue(Rect position) {
			...
			pos.x += pos.width;
			pos.width = position.width - 70 - 80;
			using (new AFStyles.EditorLabelWidth( 10 ))
				EditorGUI.PropertyField( pos, targetProp, new GUIContent( " " ) );
		...
		protected virtual float DrawFrom_Height() {
			...
			if ( fromProp.isArray && fromProp.arraySize == 0 ||
			     !fromProp.isArray && fromProp.objectReferenceValue == null )
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
			...
			if ( !AFPreviewUtils.isActive && !Application.isPlaying ) {
				...
				height += DrawToggles_Height() + AFStyles.VerticalSpace;
